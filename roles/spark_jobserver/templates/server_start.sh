...
if [ "$SPARK_MASTER" ]; then
    sed -i -e "s|master =.*|master = \"spark://$SPARK_MASTER\"|"   /opt/jobserver/jobserver.conf
fi
...
JAVA_OPTS="-Xmx256m -Xms128m -XX:MaxDirectMemorySize=256M
...
java -cp $CLASSPATH $GC_OPTS $JAVA_OPTS $LOGGING_OPTS $CONFIG_OVERRIDES $MAIN $conffile 2>&1 # && echo $! > $PIDFILE
